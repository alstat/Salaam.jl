...
macro partofspeech(name, parent, code, desc, ar_label)
    esc(quote
        ...
        $name(x) = $name(x, $desc, $ar_label)
        $name() = $name($code, $desc, $ar_label)
    end)
end

macro partofspeechparam(name, parent, data, desc, ar_label)
    esc(quote
        ...
        $name(x) = $name(x, x.desc * " " * $desc, $ar_label)
    end)
end
...
# Global type
# -----------------------------------------
@partofspeech NotApplicable AbstractFeature :na "Not applicable" ""
@partofspeech Undefined AbstractFeature :u "Not applicable" ""

# -----------------------------------------
# Aspects
# -----------------------------------------
@partofspeech Command AbstractAspect :asp!c "Command" ""
@partofspeech Imperfect AbstractAspect :asp!i "Imperfect" ""
@partofspeech Imperative AbstractAspect :asp!iv "Imperative" ""
@partofspeech Perfect AbstractAspect :asp!p "Perfect" ""

@partofspeechparam Aspect AbstractAspect Imperative() "aspect" ""
